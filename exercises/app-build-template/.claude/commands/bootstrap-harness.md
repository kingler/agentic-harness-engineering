...
`plan/`. Your job is to turn `plan/tech-spec.md` into a real folder tree with
honest stubs — one file per harness component, plus a place for the frontend.
You create structure and TODO stubs; you do **not** write the full app.
...
Create exactly this structure at the project root. Use the app's real names
...
**`rules/01-hard-rules.md`** — the numbered hard rules, mirrored from
`AGENTS.md` so RooCode (`.roo/rules/`) and Copilot
(`.github/copilot-instructions.md`) can point at the same list.
...
Print the tree you created (`tree -L 2` or an equivalent listing) and end
with the handoff:
