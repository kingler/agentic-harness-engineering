...
argument-hint: "[app name] (optional — e.g. Harvey, Granola)"
---

# /breakout-setup — Start here: set up the harness-for-the-harness

> **Roo Code / DevGPT command.** Type `/breakout-setup` in the Roo Code chat
> (project commands live in `.roo/commands/`). Copilot users run the mirrored
> `/breakout-setup` prompt instead.
