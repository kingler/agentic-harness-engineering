...
If your group hasn't already chosen an app, pick one from the 3 profiles in `exercises/ai-apps/`:

| Option | App | Best for |
|--------|-----|----------|
| A | **Harvey** — legal AI platform | Groups interested in workflows, citations, and strict data boundaries |
| B | **Claude for Financial Services** — regulated finance | Groups interested in verification, lineage, and enterprise governance |
| C | **Granola** — AI meeting notepad | Groups interested in deterministic triggers and "harness as workflow" |

**Tip:** Pick the app whose domain your group knows best. Your first-hand intuition about what an analyst, lawyer, or meeting host actually needs makes the reverse-engineering faster and richer.
...
- Harvey's per-matter isolation → the data boundary *is* the sandbox
- Harvey's citation requirement → determinism layered on top of generation via a hook
- Claude Financial Services' "model is not the source of truth for numbers" → guardrail enforced in tools + hooks, not the prompt
- Claude Financial Services living inside Excel / PowerPoint → surface choice as a product-defining decision
- Granola's meeting-end generation pass → trigger as the product, not chat
- Granola's templates → user-editable system prompts hiding in plain sight
